define('EpisodeDayRate',['Raphael','Triangle'], function (Raphael,Triangle) {
   ...
   /* var rateData = {
    ...
    //循环画Y刻度
    for (var m=0;m<9;m++){
        paper.text(40, startY+(chatHeight/8)*m, 250 - m * 25).attr({"text-anchor": "end","font-size":7});
    }
    //白天晚上标识
    ...
    for (var i = 0; i < rateList.length; i++) {
        ...
        if (data.max>250)
            data.max=250;
        //第一个点的位置X
        var posX = startX + start*(chatWidth/(24*60));
        var maxY=chatHeight+startY-((chatHeight/8/25)*(data.max-50));
        var minY=chatHeight+startY-((chatHeight/8/25)*(data.min-50));
        var avgY=chatHeight+startY-((chatHeight/8/25)*(data.avg-50));
