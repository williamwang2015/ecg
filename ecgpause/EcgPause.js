define('EcgPause',['Raphael','Triangle'], function (Raphael,Triangle) {
    ...
    /* var rateData = {
        ...
        rateList: [{rateIndex: 24,nums:2,day:1},
            {rateIndex: 35,nums:6,day:0},
            {rateIndex: 46,nums:3,day:1},
            {rateIndex: 47,nums:5,day:1}
        ]
    };*/
    ...
    //循环画Y刻度
    for (var m=0;m<5;m++){
        paper.text(36, startY+(chatHeight/4)*m, unitY*(4-m)).attr({"text-anchor": "end"}).attr({"font-size":7});
    }
     //白天晚上标识
  //  paper.text(18,startY+58,'停搏').attr({'font-size':7});
    paper.text(16,startY+70,'次数/小时').attr({'font-size':7});
