define(['d3','Triangle'], function (d3,Triangle) {
    ...
    var chatWidth = 180;
    //高度根据宽度自动计算
    var chatHeight = chatWidth / 12 * 6 ;

    // 定义x和y比例尺
    x = d3.scaleLinear().domain([0, 600]).range([0, chatWidth]),
    ...
    //竖线
    svg.selectAll(".vlines").data(d3.range(11)).enter().append("line").attr("stroke", "#9B9B9B").attr("stroke-width", 0.4)
    ...
    svg.selectAll(".hlines").data([-1.5,-1,-0.5,0,0.5,1,1.5]).enter().append("line").attr("stroke", "#9B9B9B").attr("stroke-width", 0.4)
       ...
       datas.forEach(function (d, i) {
        ...
        //右侧底部秒数
        svg.append("text").attr('x', x(550)).attr('y', y(-1.2)).text(seconds + ' sec').attr('font-size', 6).attr('text-anchor', 'end');
        ...
        svg.append('path')
                ...
                .attr("stroke-width", 0.4);
        //小图秒数
        svg.append("text").attr('x', minX(2200)).attr('y', miniChatHeight+chatHeight).text(miniChatSeconds + ' S').attr('font-size', 7).attr('text-anchor', 'end');
