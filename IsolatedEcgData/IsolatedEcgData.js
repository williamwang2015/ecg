define('IsolatedEcgData',['Raphael','Triangle'], function (Raphael,Triangle) {
    ...
    var chatWidth=180;
	var chatHeight=chatWidth/(seconds/0.2)*6*nums;//90
    ...
    for (var m=1;m<maxLine*5;m++){
        for (var n=1;n<30*nums;n++){
            if (m%5==0||n%5==0){
              ...
              //  paper.circle(pointX, pointY, 0.01).attr({"fill": "#AEAEAE", "stroke": "#AEAEAE"});
               //paper.rect(pointX, pointY, 0.001,0.001).attr({"fill": "#000", "stroke": "#000"});
    ...
    if (nums==1){//取中间2.4秒数据
       ecgData.push(ecgDatas.slice(3300/4,5700/4));
    }
    ...
    if (nums==1){
        ...
        //中间2.4秒画蓝色背景
      //  paper.rect(startX+miniChatWidth/9*12,(chatHeight+offsetY),miniChatWidth/9*2.4,miniChatHeight).attr({"fill": "#ECF8FC","stroke": "#ECF8FC"});
            paper.rect(startX+miniChatWidth/9*(9-seconds)/2,(chatHeight+offsetY),miniChatWidth/9*seconds,miniChatHeight).attr({"fill": "#ECF8FC","stroke": "#ECF8FC"});
     
        
        //外边矩形框
        paper.rect(startX,(chatHeight+offsetY),miniChatWidth,miniChatHeight).attr({"stroke-width":0.6});
        //画30S文字
        paper.text(miniChatWidth-2,chatHeight+offsetY+miniChatHeight-5,'9 S').attr({"text-anchor":"end","font-size":"7"});
        //画心电图 9S 2500个点
        var pointStr='';
        ...
        for (var k=0;k<ecgDatas.length;k++){
                ...
                if (ecg<-1.5) ecg=-1.5;
                //计算xy坐标
                var ecgX=startX+chatWidth/(9000/4)*k;
