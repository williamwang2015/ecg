define(['d3','Triangle'], function (d3,Triangle) {
	 ...
	 return function (instanceData) {	
        ...
        var xScale = d3.scaleLinear()
                .domain([0,48*60/20]).range([paddingLeft, w-108]);
        var xScaleAf = d3.scaleLinear()
                .domain([0,48*60]).range([paddingLeft, w-108]);
        ...
        attr("stroke-width",0.6).attr("stroke","#000").attr("fill","#000");
        //画竖分割线
    	svg.selectAll(".hlines").data(d3.range(3,147,3).filter(function(sn) { if (sn!==36 &&sn!==108) return true; else return false })).enter().append("line").attr("stroke", "#A6A6A6").attr("stroke-width", 0.4)
            .attr("x1", xScale).attr("y1", paddingTop)
            .attr("x2", xScale).attr("y2", paddingTop+h);
    	svg.selectAll(".hlines").data([36,108]).enter().append("line").attr("stroke", "#A6A6A6").attr("stroke-width", 0.4)
            .attr("x1", xScale).attr("y1", paddingTop)
            .attr("x2", xScale).attr("y2", paddingTop+h);
   		//画时间刻度
    	svg.selectAll(".time").data([6*3,12*3,18*3,24*3,30*3,36*3,42*3]).enter().append("text").attr("text-anchor", "middle").attr('font-size', 6)
        ...
        svg.append("text").attr("text-anchor", "middle").attr('font-size', 6)
                .attr("x", paddingLeft+(w-paddingLeft-108)/4).attr("y", h+paddingBottom+paddingTop).text(rateData.date1);
        //第二天
        svg.append("text").attr("text-anchor", "middle").attr('font-size', 6)
                .attr("x", paddingLeft+(w-paddingLeft-108)/4*3).attr("y", h+paddingBottom+paddingTop).text(rateData.date2);
        //右边内容
