define(['d3','Triangle'], function (d3,Triangle) {
   ...
   //处理越界数据
       ...
       svg.append("text").attr('x', x(50)).attr('y', y(-1.4)).text('25mm/s 10mm/mV').attr('font-size', 6).attr('text-anchor', 'start');
        //右侧底部秒数
        svg.append("text").attr('x', x(850)).attr('y', y(-1.4)).text(seconds + ' s').attr('font-size', 6).attr('text-anchor', 'end');
    ...
    //标注信息
     if (annos!=null){
    	...
    	for(j = 0,len=annos.length; j < len; j++) {
    	    if (annos[j][0]>2018) break;
          	if (annos[j][0]>=1168 && annos[j][0]<2018)//4.675~8.075秒之间
          	    anno.push(annos[j]);
   ...
   svg.selectAll('.text').data(anno).enter().append('text').attr("x",function(d,i){
    ...
    svg.selectAll('.text').data(anno).enter().append('text').attr("x",function(d,i){
        if (d[2]>0){
        if (i>0){
            return cx((d[0]-anno[i-1][0])/2+anno[i-1][0]-1168);
        }else{
            return cx((d[0]-1168)/2);
        }
        }
        return  0;
